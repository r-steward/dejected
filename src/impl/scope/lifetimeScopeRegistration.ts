...
export type ContainerRegistration<T = unknown> =
  | TypeRegistration<T>
  | ValueRegistration<T>;

export const isTypeRegistration = <T>(
  r: ContainerRegistration<T>
): r is TypeRegistration<T> => r.type === "Class" || r.type === "Factory";
export const isSingletonRegistration = <T>(r: TypeRegistration<T>): boolean =>
  r.scope === "singleton";
